############################################# Figure4g-j boxplots comparing CD8+ T cell subtype frequencies between response group#############
...
cells=as.character(unique(df_plot$CellSubset))[15:29]
...
cells=as.character(unique(df_paired$CellSubset))[15:29]
...
p=plot_celltype_proportion(stat1,'CD8.TRM.4(TIGIT+)')
p
p=plot_celltype_proportion(stat1,'CD8.Effector.1(IFNG+)')
p
p=plot_celltype_proportion(stat1,'CD8.TEM.1(CD44hi)')
p
p=plot_celltype_proportion(stat1,'CD8.TEM.2(GZMK+)')
p
